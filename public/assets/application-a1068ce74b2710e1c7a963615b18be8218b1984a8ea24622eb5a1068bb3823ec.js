...
(function() {
  var display_time_since;

  App.room = App.cable.subscriptions.create("WebNotificationsChannel", {
    received: function(data) {
      return $('#messages').prepend(display_time_since(data['time']));
    }
  });
  ...
  $(document).on('turbolinks:load', function() {
    ...
    return $('#messages').prepend(display_time_since(time));
  });

  window.do_ajax = function() {
    return $.ajax({
      url: '/' + Date.now() + '.json'
    }).done(function(data) {
      return $('#messages').prepend(display_time_since(data.time));
  ...
  display_time_since = function(time) {
    return '<p>' + (Date.now() - time) + ' ms</p>';
  };
